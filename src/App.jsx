...
function App() {
  ...
  return (
    <div className={`h-full
                      ${darkMode ? "bg-[#121212] text-gray-100" : "bg-[#ffffff] text-gray-900"}`}>
      <Header darkMode={darkMode} setDarkMode={setDarkMode} />
      <div data-aos="fade-up" className={`hero-section relative w-full py-12
        ...
        <div className={`relative z-10 pb-20 `}>
            <div data-aos="fade-up" className={`my-5`}>
              <h1 className={`font-bold lg:text-5xl text-3xl 
                            `}>Elevate your look </h1>
              <h1 className={`font-bold lg:text-5xl text-3xl`}>with <span className={`font-bold ${darkMode ? "text-[#C3A360]" : "text-[#C3A360]"}`}>RIIKEHDELUXE</span></h1>
            </div>
            <div data-aos="fade-up">
              ...
              <p className={`lg:text-xl text-lg font-semibold border-b-1 pb-2 w-2/3
                            ${darkMode ? "border-[#C3A360]" : "border-[#C3A360]"}`}>Crafted to define your style</p>
            </div>
      ...
      <div data-aos="fade-up" className={`search-area md:flex justify-between items-center py-10 lg:px-20 md:px-15 px-5 `}>
        ...
        <div className={`categories flex`}> 
          <select data-aos="fade-left" className={`focus:outline-none ring-2 md:pr-10 pl-3 py-1 lg:w-70 md:w-50 w-half rounded-lg font-semibold cursor-pointer
                          ${darkMode ? "ring-[#C5A253]" : "ring-[#C5A253]"}
                          `}
                value={category} 
      ...
      <div className={`grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 
                        md:gap-10 gap-13 cursor-pointer lg:px-20 md:px-15 px-5 py-7
                    ${darkMode ? "bg-[#121212] text-gray-100" : "bg-[#ffffff] text-gray-900"}`}>
        {
          filteredItems.map((p) => (
            <div data-aos="fade-up">
              <div className={`shadow-lg rounded-t-4xl transition-transform rounded-b-2xl
                ...
                <div className={`text-center pt-5 border-1 border-[#C5A253] rounded-b-2xl`}>
                  ...
                  <p className={`py-3 mt-7 border-1 border-[#C5A253] rounded-b-2xl
                                ${darkMode ? " text-[#ffffff]" : " text-[#121212]"}`}>
      ...
      <div className={`contact-button fixed z-20 bottom-20 lg:right-20 md:right-15 right-5`}>
        <button className={`px-5 py-2 rounded-4xl cursor-pointer shadow-md transform duration-500 ease-in-out
                          ${darkMode ? "hover:bg-gray-900 bg-[#C3A360] hover:text-[#C3A360] text-gray-900 shadow-gray-600 hover:border-2" : "hover:bg-gray-100 bg-[#C3A360] hover:border-2 hover:text-[#C3A360] text-white shadow-gray-600"}`}>
      ...
      <footer className={`sticky bottom-0 flex justify-between items-center lg:py-3 py-2 lg:px-20 md:px-15 px-5 shadow-2xl z-20
                            ${darkMode ? "bg-[#121212] text-[#ffffff] shadow-gray-600" : "bg-[#ffffff] text-[#121212] shadow-gray-600"}`}>
        <nav>
          <a href="#">
